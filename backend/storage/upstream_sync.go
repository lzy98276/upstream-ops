		...
		for i := range list {
			...
			if list[i].ID == 0 {
				...
				if err := tx.Model(&UpstreamSyncAccount{}).Where("id = ?", list[i].ID).Updates(map[string]any{
					...
					"source_kind":        list[i].SourceKind,
					"source_channel_id":  list[i].SourceChannelID,
					"source_group_id":    list[i].SourceGroupID,
					"source_group_name":  list[i].SourceGroupName,
					"gateway_group_id":   list[i].GatewayGroupID,
					"gateway_rate_mode":  list[i].GatewayRateMode,
					"gateway_rate_min":   list[i].GatewayRateMin,
					"gateway_rate_max":   list[i].GatewayRateMax,
